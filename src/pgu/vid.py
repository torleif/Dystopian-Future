...
class Vid:
    ...
    def __init__(self):
        ...
        self.edata = [None for x in xrange(0,256)]
        self.sprites = _Sprites()
        self.backsprites = _Sprites()
    ...
    def run_codes(self,cdata,rect):
        ...
        for y in range(y1,y1+h):
            for x in range(x1,x1+w):
                n = clayer[y][x]
                if n in cdata:
                    clist.append((n,t,x,y))

        sortedList = sorted(clist, key=lambda clist: clist[0])
    ...
    def code_events(self, edata = {}):
        """sets events for the code layer

        <pre>Vid.code_events(edata={})</pre>
        ...
        for _z in (0,): 
            if s.groups != 0:
                ...
                while y < cb:
                    ...
                    while x < cr:
                        xx = x/tw
                        if yy >= h or xx >= w or yy < 0 or xx < 0:
                            break
                        t = tiles[layer[yy][xx]]
                        ct = self.clayer[yy][xx]
                        if (s.groups & t.agroups)!=0:
                            d = math.hypot(rect.centerx-(xx*tw+tw/2),
                                rect.centery-(yy*th+th/2))
                            hits.append((d,t,xx,yy))
                        if ct in self.edata:# code layer data
                            if self.edata[ct][0] in self.images:
                                r = Rect(xx*tw,yy*th,tw,th)
                                self.edata[ct][1](self, r, s)
                        x += tw
                    y += th
    ...
    def make_image_white(self, image, pos = (32, 32)):
        outsurface = pygame.surface.Surface(pos, SRCALPHA, 32)
        for x in range(pos[0]):
            for y in range(pos[1]):
                c = image.get_at((x, y))
                if c.a == 255:
                    outsurface.set_at((x,y), pygame.Color(255,255,255))
        return outsurface
