...
class Tilevid(Vid):
    ...
    def paint(self,s):
        ...
        backsprites = self.backsprites

        for sm in backsprites:
            sm.irect.x = sm.rect.x-sm.shape.x+sm.renderoffset[0]
            sm.irect.y = sm.rect.y-sm.shape.y+sm.renderoffset[1]
            s.blit(sm.image,(sm.irect.x-ox,sm.irect.y-oy))
            sm.updated=0
            sm._irect = Rect(sm.irect)
        ...
        for s in sprites:
            s.irect.x = s.rect.x-s.shape.x+s.renderoffset[0]
            s.irect.y = s.rect.y-s.shape.y+s.renderoffset[1]
